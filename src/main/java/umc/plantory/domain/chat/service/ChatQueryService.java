...
public class ChatQueryService implements ChatQueryUseCase {
    ...
    @Override
    public List<ChatResponseDto.ChatResponse> findLatestChatsByMemberId(Long memberId) {
        Member member = memberRepository.findById(memberId)
                .orElseThrow(() -> new EntityNotFoundException("해당 사용자를 찾을 수 없습니다."));

        return chatJpaRepository.findTop6ByMemberOrderByCreatedAtDesc(member)
                .stream().map(
                        chat -> new ChatResponseDto.ChatResponse(
    ...
    @Override
    public List<ChatResponseDto.ChatResponse> findBeforeChatsByMemberId(Long memberid, LocalDateTime before) {
        Member member = memberRepository.findById(memberid)
                .orElseThrow(() -> new EntityNotFoundException("해당 사용자를 찾을 수 없습니다."));
